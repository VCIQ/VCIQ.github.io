...
const SHIFT_MARKERS = /转向|转而|改为|重新聚焦|重心转|从.+到|pivot|shift(?:ed|ing)?|move(?:d)? from|instead of/iu;
...
export function assessPersonViewChange(
  ...
  const directEvents = events
    .map((event) => ({
      event,
      material: directExpressionMaterial(event),
      dateMs: eventDateMs(event, person.updatedAt),
    }))
    .filter((item): item is typeof item & { material: PersonMaterial } => Boolean(item.material && item.dateMs))

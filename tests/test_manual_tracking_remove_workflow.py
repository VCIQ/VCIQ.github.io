...
WORKFLOW = ROOT / ".github" / "workflows" / "manual-tracking-remove.yml"


class ManualTrackingRemovalWorkflowTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_removal_preserves_provenance_and_commits_only_two_config_files(self):
        self.assertIn("tools/manual_tracking_remove.py", self.text)
        self.assertIn("config/tracking_intents.json config/user_tracking.json", self.text)
    ...
    def test_changed_configuration_reuses_existing_governance_gates(self):
        self.assertIn("tests.test_manual_tracking_remove", self.text)

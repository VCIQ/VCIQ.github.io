...
function nodeCanonicalTracks(config: any) {
  return (Array.isArray(config.tracks) ? config.tracks : [])
    .filter((track: any) => track && typeof track === "object" && track.enabled !== false)
    .map((track: any) => ({
    ...
    .filter((track: any) => track.slug && track.name);
}
...
test("Python enrichment and Node snapshot validator share one canonical tracking hash", () => {
  const config = JSON.parse(fs.readFileSync("config/user_tracking.json", "utf8"));
  ...
  if (python.canonical !== nodeJson) {
    const pythonTracks = JSON.parse(python.canonical);
    ...
    for (let index = 0; index < maxTracks; index += 1) {
      ...
      const fields = ["slug", "name", "keywords", "people", "sampleCompanies"];
      const fieldDiffs: Record<string, unknown> = {};
      for (const field of fields) {
        if (JSON.stringify(nodeTrack?.[field]) === JSON.stringify(pythonTrack?.[field])) continue;
        fieldDiffs[field] = {
          node: nodeTrack?.[field],
          python: pythonTrack?.[field],
        };
      }
      differences.push({ index, nodeSlug: nodeTrack?.slug, pythonSlug: pythonTrack?.slug, fields: fieldDiffs });

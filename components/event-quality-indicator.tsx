...
export function EventQualityIndicator({ item }: { item: LiveIntelligenceEvent }) {
  ...
  const hasQuality = Boolean(item.qualityStatus) || typeof item.qualityScore === "number" ||
    totalLinks > 0 || Boolean(item.qualitySignals?.length);
  if (!hasQuality) return null;
  return (
    <div className={styles.sourceRow} aria-label="信息质量与关联证据">
      {item.qualityStatus && (
        <span className={styles.qualityBadge} data-status={item.qualityStatus}
      ...
      {totalLinks > 0 && (
        <span title="可查看的去重来源链接数；不代表独立信源数或事实已获交叉验证。">来源链接 {totalLinks}</span>
      )}
        ...
        <details className={styles.evidence}>
          ...
          <div className={styles.evidenceList}>
            {additionalLinks.map((source, index) => (
              <a href={source.url} key={`${source.url}-${index}`} target="_blank" rel="noreferrer" title={source.title}>
                <strong>{source.title || source.name}</strong>
                <span>{source.level || source.platform || source.name}<ArrowUpRight size={11} /></span>
              </a>
            ))}
          </div>
        </details>
